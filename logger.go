...
func (r *RotatedLogger) SetScheduler(spec string) {
	newCronScheduler, err := cron.NewCronScheduler(context.Background())
	if err != nil {
		slog.Error("error setting up cron scheduler", slog.String(loggerSchedulerName, err.Error()))
	}
